...
def upgrade() -> None:
    with op.batch_alter_table("users") as batch:
        batch.add_column(sa.Column("account_status", sa.String(24), nullable=False, server_default="approved"))
        batch.add_column(sa.Column("payment_required", sa.Boolean(), nullable=False, server_default=sa.false()))
        ...
        batch.create_foreign_key("fk_users_approved_by", "users", ["approved_by"], ["id"])
    op.create_table(
        "payment_submissions",
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("users") as batch:
        ...
        for name in ("is_admin", "approved_by", "approved_at", "payment_required", "account_status"):
